...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    if (self == NULL || args == NULL || !PyObject_TypeCheck(self, &Matrix61cType) || 
    !PyObject_TypeCheck(((Matrix61c*) args), &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Type Error Add");
        return NULL;
    }


    matrix* newmat;
    if (allocate_matrix(&newmat, self->mat->rows, self->mat->cols)){
        PyErr_SetString(PyExc_RuntimeError, "Failed to allocate matrix");
        return NULL;
    }
    if(add_matrix(newmat, self->mat, ((Matrix61c*) args)->mat)){
        PyErr_SetString(PyExc_ValueError, "Dimensions Add");
        return NULL;
    }
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    if (self == NULL || args == NULL || !PyObject_TypeCheck(self, &Matrix61cType) || 
    !PyObject_TypeCheck(((Matrix61c*) args), &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Type Error Sub");
        return NULL;
    }
    ...
    if(sub_matrix(newmat, self->mat, ((Matrix61c*) args)->mat)){
        PyErr_SetString(PyExc_ValueError, "Dimensions Sub");
        return NULL;
    }
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    if (self == NULL || args == NULL || !PyObject_TypeCheck(self, &Matrix61cType) || 
    !PyObject_TypeCheck(((Matrix61c*) args), &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Type Error Mul");
        return NULL;
    }

    matrix* newmat;
    if (allocate_matrix(&newmat, self->mat->rows, ((Matrix61c*) args)->mat->cols)){
        PyErr_SetString(PyExc_RuntimeError, "Failed to allocate matrix");
        return NULL;
    }

    if(mul_matrix(newmat, self->mat, ((Matrix61c*) args)->mat)){
        PyErr_SetString(PyExc_ValueError, "Dimensions Mul");
        return NULL;
    }

    Matrix61c* result = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    result->mat = newmat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(self->mat->rows), PyLong_FromLong(((Matrix61c*) args)->mat->cols));
    return (PyObject*) result;
}
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    if (self == NULL || !PyObject_TypeCheck(self, &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Type Error Mul");
        return NULL;
    }
    ...
    if(neg_matrix(newmat, self->mat)){
        PyErr_SetString(PyExc_ValueError, "Dimensions neg");
        return NULL;
    }
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    if (self == NULL || !PyObject_TypeCheck(self, &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Type Error Abs");
        return NULL;
    }
    ...
    if(abs_matrix(newmat, self->mat)){
        PyErr_SetString(PyExc_ValueError, "Dimensions Abs");
        return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    if (self == NULL || !PyObject_TypeCheck(self, &Matrix61cType) || !PyLong_Check(pow)){
        PyErr_SetString(PyExc_TypeError, "Type Error Pow");
        return NULL;
    }
    matrix* newmat;
    if (allocate_matrix(&newmat, self->mat->rows, self->mat->cols)){
    ...
    if(pow_matrix(newmat, self->mat, PyLong_AsLong(pow))){
        PyErr_SetString(PyExc_ValueError, "Value Pow");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (!PyArg_UnpackTuple(args, "row, col, val", 3, 3, &row, &col, &val) || !PyLong_Check(row) || 
    !PyLong_Check(col) || !(PyLong_Check(val) || PyFloat_Check(val))){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments in set");
    }
    long r = PyLong_AsLong(row);
    long c = PyLong_AsLong(col);

    if (r >= self->mat->rows || c >= self->mat->cols){
        PyErr_SetString(PyExc_ValueError, "Invalid index in set");
    }
    if(PyFloat_Check(val)){
        set(self->mat, r, c, PyFloat_AsDouble(val));
    } else {
        set(self->mat, r, c, PyLong_AsDouble(val));
    }
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if(!PyArg_UnpackTuple(args, "rows and cols", 2, 2, &row, &col) || !PyLong_Check(row) 
    || !PyLong_Check(col)){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments in get");
    }
    ...
    if (r >= self->mat->rows || c >= self->mat->cols){
        PyErr_SetString(PyExc_IndexError, "Invalid index in get");
    }
    double result = get(self->mat, r, c);
